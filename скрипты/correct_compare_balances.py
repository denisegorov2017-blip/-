...
def parse_inventory_file_correctly(csv_file):
    """
    Правильно парсит файл с отчетом о товародвижении и извлекает начальные остатки для номенклатур.
    
    Args:
        csv_file: Путь к CSV файлу с отчетом
        
    Returns:
        Словарь с номенклатурами и их начальными остатками
    """
    ...
    for idx, row in df.iterrows():
        if pd.notna(row[0]) and str(row[0]).strip() == "Номенклатура":
            header_found = True
    ...
    # Ищем строки с номенклатурами после заголовков
    for idx in range(header_row_index + 1, len(df)):
        ...
        is_nomenclature = (
            pd.notna(row[4]) and str(row[4]).strip() and  # Проверяем колонку с начальным остатком (индекс 4)
        ...
        if is_nomenclature:
            try:
                # Очистка и преобразование остатка из колонки E (индекс 4)
                balance_str = str(row[4]).strip().replace(',', '.')
                balance = float(balance_str)
                balances[row_str] = balance
                print(f"Найдена номенклатура: {row_str}, остаток: {balance}")
            except (ValueError, IndexError) as e:
                print(f"Ошибка при парсинге остатка для {row_str}: {e}")
...
def compare_balances(balances1, balances2, file1_name, file2_name):
    """
    Сравнивает начальные остатки из двух источников.
    
    Args:
        balances1: Словарь с остатками из первого источника
        balances2: Словарь с остатками из второго источника
        file1_name: Название первого файла
        file2_name: Название второго файла
...
def main():
    ...
    try:
        ...
        main_balances = parse_inventory_file_correctly(main_report_file)
        print(f"Извлечены остатки по {len(main_balances)} номенклатурам из основного отчета")
        
        print("\nПарсинг предварительного отчета...")
        prelim_balances = parse_inventory_file_correctly(prelim_report_file)
        ...
        if not comparison_df.empty:
            ...
            output_file = os.path.join(project_root, "результаты", "детальное_сравнение_остатков_v2.csv")
